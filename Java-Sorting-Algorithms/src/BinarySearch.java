public class BinarySearch {
     ...
     * @return
     */
    public static int BinarySearching(int[] arr, int left, int right, int query) {
        ...
        if (query < arr[mid]) {
            return BinarySearching(arr, left, mid - 1, query);
        }
        // Or else we search in the right half
        return BinarySearching(arr, mid + 1, right, query);
    }

    public static void main(String[] args) {
        int[] array = { -1, 0, 1, 2, 3, 4, 7, 9, 10, 20 };
        int query = 8;
        int index = BinarySearching(array, 0, array.length - 1, query);
